...
class NativeRPCStub {
	...
	constructor() {
		...
		this.callbackIdCount = Math.floor(Math.random() * 134217728);
	...
	call<T>(rpcSignature: string, params?: Record<string, NativeRPCParamValue>) {
		...
		return new Promise<T>((resolve, reject) => {
			...
			win?.webkit?.messageHandlers?.bridge.postMessage(request);
		});
	}

	private onReceive(response: NativeRPCResponse) {
		if (response._meta.event) {
			this.emitter.emit(`${response.service}.${response._meta.event}`, response.data);
			return;
		}
		...
		if (storedCall) {
			if (response.code !== 1000) {
				storedCall?.reject?.(new Error(response.message ?? `${JSON.stringify(response._meta)} call error`));
			} else {
